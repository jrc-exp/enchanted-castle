<template>
  <div class="pin-code-container flex flex-col items-center gap-4 my-4">
    ...
    <div class="flex justify-center gap-2">
      <input v-for="index in answer.length" :key="index" :class="inputSizeClass(index)" type="text" maxlength="1"
        v-model="pin[index - 1]" @input="handleInput(index, $event)" @keydown="handleKeyDown(index, $event)"
        :ref="`pinInput${index}`" @paste.prevent />
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    inputSizeClass(index) {
      const base = `
        ...
        `
      // 45, 15, 25, 35, 10
      const vals = [0, 20, 12, 16, 16, 12];
      const val = vals[index % vals.length];
      const vals2 = [0, 20, 16, 16, 20, 12];
      const val2 = vals2[index % vals.length];
      console.log(val, index)
      return base + ` h-${val2} w-${val}`;
    },
    mounted() {
      this.$refs.pinInput1[0].focus();
    },
